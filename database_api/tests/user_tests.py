...
async def test_create_valid_user(clear_test_users_db, valid_user_data):
    ...
    async with AsyncClient(app=app, base_url="http://localhost/db/") as async_client:
        response = await async_client.post("create_user/", json=valid_user_data)
...
async def test_get_user_by_email(test_db_with_one_test_user, user_to_insert):
    ...
    async with AsyncClient(app=app, base_url="http://localhost/db/") as async_client:
        response = await async_client.get(f"get_user_by_email/?email={user_to_insert['email']}")
...
async def test_delete_user(test_db_with_one_test_user, user_to_insert):
    ...
    async with AsyncClient(app=app, base_url="http://localhost/db/") as async_client:
        response = await async_client.delete(f"delete_user/?email={user_to_insert['email']}")
...
@pytest.mark.asyncio
async def test_update_user_password(test_db_with_one_test_user,
                                    user_to_insert,
