...
@auth_router.post("/register",
                  ...
                  status_code=status.HTTP_200_OK)
async def register(user_in: UserIn, users: UserRepository = Depends(get_user_repository)):
    user_ = await users.get_by_email(user_in.email)

    if user_:
        return HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This email is busy")
